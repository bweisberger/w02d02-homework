...
// Use the huge example array below to do the following.
//
// Use .filter() to create a version of the array that has only the "English" objects (i.e. the Language property is equal to en).
// Using .forEach() with some control flow, print out only strings for the French objects (Language === 'fr').
// Use .every() to confirm that no TagID is 0.
// Use .some() to determine whether or not there is an object with TagID === 46
// Use .map() to create a new array based on the one below. The new array will have strings formatted with the TagName and then the Language in parentheses. For example, the first element in the array should be the following string:
// Walt Disney World (en)
// Remember that some array methods return new copies of arrays instead of changing their values in place (i.e. instead of mutating them).
//
